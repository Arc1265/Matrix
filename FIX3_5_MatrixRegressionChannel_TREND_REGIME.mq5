//+------------------------------------------------------------------+
//| FIX3_5_MatrixRegressionChannel_TREND_REGIME.mq5                   |
//| FIX3_5: режимный движок поверх квадратичного МНК-ядра FIX2.      |
//|                                                                  |
//| FIX3_5 (обратная связь: "много пропусков, флэт вместо тренда"):   |
//|  - средний детектор: тренды длиной 10-25 баров проваливались     |
//|    между медленным окном (40 баров разбавляют движение) и        |
//|    дорожкой разворота (нужен контекст свежего тренда). Теперь    |
//|    устойчивое движение быстрого окна - |S_fast|>=InpMedS и       |
//|    |ER_fast|>=InpMedER на InpMedBars закрытых барах ПОДРЯД -     |
//|    признаётся трендом без ожидания медленного окна.              |
//|  - на обоих CSV (2022-2025): покрытие трендами 29% -> ~52%       |
//|    времени, мусорные эпизоды остались на 3-4%, PF виртуальных    |
//|    сделок 1.18 -> 1.11 (зоны включают и более слабые тренды -    |
//|    для советника входы можно будет фильтровать по силе).         |
//|                                                                  |
//| FIX3_4 (по CSV прогона FIX3_3, XAUUSD H1 2022-2025):              |
//|  - выход по пересечению центральной линии теперь требует         |
//|    InpExitTLConfirmBars закрытий за линией ПОДРЯД (по умолчанию  |
//|    3). Один бар за линией - это откат внутри тренда, а не конец: |
//|    одиночный кросс давал среднюю сделку +0.27$, подтверждение    |
//|    3 бара: +0.54$, PF 1.11 -> 1.18. Значение 0 отключает этот    |
//|    выход полностью (выход только по концу режима: +0.72$/сделка).|
...
#define MRC3_VERSION      "FIX3_5_TREND_REGIME"
...
input group "--- Средний детектор устойчивого тренда (FIX3_5) ---"
input bool   InpMedEnable   = true;   // Ловить тренды средней длины быстрым окном
input double InpMedS        = 2.5;    // |S| быстрого окна (устойчивое, не экстремум)
input double InpMedER       = 0.30;   // Минимальный |ER| быстрого окна
input int    InpMedBars     = 4;      // Закрытых баров подряд для подтверждения
...
input int    InpExitTLConfirmBars    = 3;    // Закрытий за линией подряд для выхода (0 = выход отключен)
...
int      g_decel_cnt         = 0;
int      g_fade_cnt          = 0;
int      g_tlcross_cnt       = 0;   // FIX3_4: закрытий за центральной линией подряд
int      g_med_dir           = 0;   // FIX3_5: направление кандидата среднего детектора
int      g_med_cnt           = 0;   // FIX3_5: подряд подтверждённых баров
                      ...
                      const double &low[], const double &close[], const bool allow_alert)
{
   ...
   //--- 1а+) FIX3_5: средний детектор - устойчивое движение быстрого окна.
   // Ловит тренды 10-25 баров, которые размываются в медленном окне.
   int med = 0;
   if(InpMedEnable)
   {
      int mraw = 0;
      if(sf >= InpMedS && erf >= InpMedER)        mraw = REGIME_UP;
      else if(sf <= -InpMedS && erf <= -InpMedER) mraw = REGIME_DOWN;

      if(mraw != 0 && mraw == g_med_dir) g_med_cnt++;
      else if(mraw != 0) { g_med_dir = mraw; g_med_cnt = 1; }
      else               { g_med_dir = 0;    g_med_cnt = 0; }

      if(g_med_cnt >= InpMedBars) med = g_med_dir;
   }
   ...
   // 3б+: FIX3_5 - средний детектор устойчивого тренда
   if(med != 0 && g_regime != med)
      g_regime = med;
   ...
   if(g_open_dir != 0 && g_regime == g_open_dir)
   {
      ...
      // FIX3_4: одиночное закрытие за линией - откат, а не конец тренда.
      // Выход только после InpExitTLConfirmBars закрытий за линией подряд.
      if(InpExitOnTrendlineCross && InpExitTLConfirmBars > 0)
      {
         const bool crossed = (g_open_dir > 0 ? close[bar] < BufTrend[bar]
                                              : close[bar] > BufTrend[bar]);
         if(crossed) g_tlcross_cnt++;
         else        g_tlcross_cnt = 0;

         if(g_tlcross_cnt >= InpExitTLConfirmBars) exit_reason = "TRENDLINE_CROSS";
      }
...
int OnInit()
{
   if(InpPeriodSlow < 10 || InpPeriodFast < 6 ||
      ...
      InpMedS < 0.5 || InpMedER < 0.0 || InpMedER > 1.0 || InpMedBars < 2 ||
      InpConfirmBars < 1 || InpEntryGraceBars < 1 ||
      InpBreakoutLookback < 2 || InpExitDecelBars < 1 || InpExitTLConfirmBars < 0 ||
